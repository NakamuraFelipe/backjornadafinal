    ...
    try:
        with get_db_connection() as conn:
            with conn.cursor() as cur:
                ...
                # 1 — Buscar IDs da rua
                cur.execute("SELECT id_rua FROM rua WHERE nome_rua LIKE %s", (like,))
                ruas = cur.fetchall()
                id_ruas = [r["id_rua"] for r in ruas]

                # 2 — Buscar IDs da localizacao
                id_localizacoes = []
                if id_ruas:
                ...
                # Converter rows → OBJETO AllLeads
                for row in rows:
                    leads.append(AllLeads(
        ...
        print("Erro em buscar_all_leads:", e)
        raise e
